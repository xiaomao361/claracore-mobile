...
- API keys are stored in ThisDeviceOnly iOS Keychain items on the user's device.
...
- Conversation content is sent to a remote model provider only when the user configures a model API key, accepts the external model processing notice, and starts an import or organization action that requires remote reflection.
...
- API keys in ThisDeviceOnly iOS Keychain items.

The app does not upload this local database to ClaraCore servers.

The local database is stored in the app's Application Support container and is marked to be excluded from iCloud and iTunes backups. This backup exclusion applies to the local SQLite database that contains imported source text, Archives, memories, Context Cards, Shared Lines, and import history.
...
If the user saves a default model configuration and accepts the external model processing notice, ClaraCore Mobile can send imported conversation segments and derived draft context to the configured OpenAI-compatible model endpoint for organization.
...
- Copying or sharing a complete original Archive entry when the user taps the raw text copy or share action.
...
Users can also clear local app data from Settings. This deletes the local Archive, Inbox, memories, Shared Lines, Context Cards, model configuration, and saved model API keys, then restores the default Context Card and local-rule organization mode.
...
API keys are stored in the iOS Keychain using `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`. Local app data is stored in the app container on the user's device.
